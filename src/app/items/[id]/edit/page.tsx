...
export default function ItemDetailPage() {
  ...
  const handleRedeemWithPoints = async () => {
    console.log('handleRedeemWithPoints called');
    console.log('User:', user);
    console.log('Item:', item);
    
    if (!user || !item) {
      console.log('Missing user or item, returning');
      return;
    }
    ...
    try {
      ...
      // Fetch current profile data to get latest points
      let currentProfile = profile;
      console.log('Current profile from auth:', profile);
      
      if (!currentProfile) {
        console.log('Profile not available from auth, fetching from database...');
        ...
        if (profileError) {
          ...
          alert('Failed to fetch your profile data. Please try again.');
          return;
        }
        currentProfile = fetchedProfile;
        console.log('Fetched profile from database:', fetchedProfile);
      }
      
      console.log('Using profile for redemption:', currentProfile);
      
      // Check if user has enough points
      const userPoints = currentProfile?.points || 0;
      console.log('User points:', userPoints, 'Item points needed:', item.points_value);
      
      if (userPoints < item.points_value) {
        alert(`You don't have enough points to redeem this item. You have ${userPoints} points, but need ${item.points_value} points.`);
        return;
      }
      ...
      const { error: pointsError } = await supabase
        .from('profiles')
        .update({ points: userPoints - item.points_value })
        .eq('id', user.id);
      ...
      console.log('Adding points to item owner...');
      const ownerCurrentPoints = item.profiles?.points || 0;
      console.log('Owner current points:', ownerCurrentPoints, 'Adding:', item.points_value);
      
      const { error: ownerPointsError } = await supabase
        .from('profiles')
        .update({ points: ownerCurrentPoints + item.points_value })
        .eq('id', item.user_id);

      if (ownerPointsError) {
        console.error('Error adding points to owner:', ownerPointsError);
        throw ownerPointsError;
      }
      console.log('Points added to owner successfully');

      // Update item status to swapped
      console.log('Updating item status to swapped...');
      ...
      alert(`Failed to redeem item: ${error?.message || 'Unknown error'}. Please try again.`);
    } finally {
  ...
  const canRedeem = user && !isOwner && item.status === 'available' && profile && (profile.points || 0) >= item.points_value;
                  ...
                  {!canRedeem && user && profile && (
                    <div className="text-center p-4 bg-amber-50 rounded-lg">
                      <p className="text-amber-800 text-sm">
                        You need {item.points_value - (profile?.points || 0)} more points to redeem this item
                      </p>
                    </div>
                  )}
                </div>
