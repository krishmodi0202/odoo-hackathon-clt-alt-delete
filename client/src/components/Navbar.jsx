...
const Navbar = () => {
  ...
  return (
    <nav className="bg-[#5c6154] fixed w-full z-10 shadow-md mt-0">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <Link to="/dashboard" className="flex-shrink-0 flex items-center">
            ...
            <div className="hidden md:block">
              <div className="ml-10 flex items-baseline space-x-4">
                <Link
                  to="/dashboard"
                  className="text-white hover:bg-[#4a4e42] px-3 py-2 rounded-sm text-sm font-light transition-colors"
                >
                  Dashboard
                </Link>
                <Link
                ...
                <Link
                  to="/requests"
                  className="text-white hover:bg-[#4a4e42] px-3 py-2 rounded-sm text-sm font-light transition-colors relative"
                >
                  Requests
                  {/* Notification badge - can be dynamically shown based on unread requests */}
                  <span className="absolute -top-1 -right-1 bg-[#e2e1d9] text-[#5c6154] text-xs rounded-full h-5 w-5 flex items-center justify-center">
                    0
                  </span>
                </Link>
                <Link
          ...
          <div className="hidden md:block">
            <button
              onClick={handleLogout}
              className="text-[#5c6154] bg-white hover:bg-[#e2e1d9] px-4 py-2 rounded-sm text-sm font-light transition-colors focus:outline-none"
            >
              Logout
            </button>
          </div>
          ...
          <div className="md:hidden">
            <button
              onClick={toggleMenu}
              className="text-white hover:bg-[#4a4e42] p-2 rounded-sm focus:outline-none transition-colors"
            >
              <svg
                className="h-6 w-6"
                xmlns="http://www.w3.org/2000/svg"
      ...
      {/* Mobile menu */}
      {isMenuOpen && (
        <div className="md:hidden bg-[#4a4e42] pb-3 pt-2">
          <div className="px-2 space-y-1">
            <Link
              to="/dashboard"
              className="text-white hover:bg-[#3a3e32] block px-3 py-2 rounded-sm text-base font-light transition-colors"
              onClick={() => setIsMenuOpen(false)}
            >
              Dashboard
            </Link>
            ...
            <button
              ...
              className="text-[#5c6154] bg-white hover:bg-[#e2e1d9] block w-full text-left px-3 py-2 rounded-sm text-base font-light transition-colors"
            >
              Logout
            </button>
          </div>
